// tether#161 — the unit half. What a body MEANS lives here.
//
// 🔴 The other half is currently MISSING, and this header used to claim it was
// covered. Ported to the phase-1 tree by tether#188; the original text read:
//
//     "that the meaning reaches a screen is asserted per call site in the
//      component tests (Settings, SkillPane, WorkspacePane, WorkspaceTree)"
//
// All four of those component tests were deleted with the old SPA in tether#174.
// The sentence is rewritten rather than kept, because a test file that says its
// weak spot is someone else's strong spot — when that someone no longer exists —
// is worse than one that says nothing: it retires the question.
//
// So, stated plainly: the discriminating power below is intact and complete AT
// THE UNIT LAYER, and `fetch → wording → rendered text` is asserted NOWHERE in
// this tree. That hop is tether#173's AC-R9. It is not closable here — phase 1
// has no rendering consumer of httpErrorMessage yet — and it is owed by the
// first wi that gives one a screen. The reason the delegation was right in the
// first place still holds: a helper that returns the right string into a
// variable nobody renders is exactly the no-op tether#161 existed to close.
//
// The nearest live call site is web/src/lib/fileTreeCache.ts, whose two arms in
// web/test/workspace-tree.spec.ts pin passthrough and degradation against each
// other — also at the unit layer, also short of a screen.
//
