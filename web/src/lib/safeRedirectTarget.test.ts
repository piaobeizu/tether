...
describe('safeRedirectTarget (tether#117 A3)', () => {
  ...
  // The round trip the feature exists for: redirectToAuth(), which lived in the
  // pre-rewrite SPA's lib/auth.ts and has not been ported onto this branch yet,
  // encodeURIComponent's the current path+search, and URLSearchParams decodes it
  // on the way back in. An encoded path must survive that unchanged — asserted
  // here against the encoding contract rather than against that caller, which is
  // why this case keeps working while the login page is rewritten.
