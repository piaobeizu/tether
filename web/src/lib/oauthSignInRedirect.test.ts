...
// real safeRedirectTarget, which is what the sign-in page navigates with. (The
// page itself is being rewritten by tether#173; the guard was moved to
// lib/safeRedirectTarget.ts by tether#185 so this contract does not wait on it.)
//
...
describe('the tether#153 sign-in redirect survives safeRedirectTarget', () => {
  ...
  it.each(corpus.entries.map(e => [e.label, e] as const))('%s', (_label, entry) => {
    // The browser's own decode path, not a hand-rolled one: the sign-in page
    // reads window.location.search through URLSearchParams, so the value handed
    // to safeRedirectTarget is whatever that produces from the Go-built href.
    ...
    // 2. It is safe. The same property safeRedirectTarget.test.ts asserts over
    //    its own generated corpus, restated over the strings this producer
    //    emits: no input may yield something that leaves the origin.
